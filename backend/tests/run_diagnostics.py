...
def test_config_loading():
    ...
    try:
        ...
        if hasattr(config, 'ANTHROPIC_API_KEY') and config.ANTHROPIC_API_KEY:
            key_preview = config.ANTHROPIC_API_KEY[:10] + "..." if len(config.ANTHROPIC_API_KEY) > 10 else "short"
...
def diagnose_issue():
    ...
    print(f"   AI Generator (real empty key): {'✅ Fails as expected' if ai_generator_real_fail else '❌ Unexpected behavior'}")
    
    # Root cause analysis
    print(f"\n🎯 ROOT CAUSE ANALYSIS:")
    
    if not config_ok and any("env file exists but is EMPTY" in issue for issue in env_issues):
