...
class TestLiveSystemComponents:
    ...
    def test_config_api_key_status(self):
        ...
        print(f"   Key preview: {config.ANTHROPIC_API_KEY[:10] + '...' if config.ANTHROPIC_API_KEY and len(config.ANTHROPIC_API_KEY) > 10 else 'EMPTY'}")
            ...
            if not config.ANTHROPIC_API_KEY:
                ...
                try:
                    ...
                    if "authentication" in str(e).lower():
                        print(f"   ✅ Correctly failed with auth error: {str(e)[:100]}...")
                        return "NO_API_KEY"
                    else:
    ...
    def test_existing_course_data(self):
        ...
        try:
            ...
            if course_count > 0:
                ...
                search_tool = CourseSearchTool(vector_store)
                result = search_tool.execute("Python", course_name=course_titles[0] if course_titles else None)
...
def run_comprehensive_live_tests():
    ...
    try:
        ...
        print(f"AI Generator: {'✅' if ai_status == 'API_KEY_OK' else '❌ (No API Key)' if ai_status == 'NO_API_KEY' else '❌'}")
        
        if query_result == "AUTH_ERROR":
            print(f"Query Flow: ❌ (Authentication Error)")
            print(f"\n🎯 ROOT CAUSE CONFIRMED:")
            print(f"   The system fails because the Anthropic API key is not configured.")
