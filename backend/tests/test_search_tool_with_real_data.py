...
class TestCourseSearchToolRealData:
    ...
    def test_execute_with_existing_courses(self):
        ...
        assert "No relevant content found" not in result1, "Should find content for 'retrieval'"
    ...
    def test_execute_with_nonexistent_query(self):
        ...
        assert "No relevant content found" in result, "Should return 'no content found' message"
    ...
    def test_execute_with_course_filter(self):
        ...
        if len(course_titles) > 1:
            ...
            if "No relevant content found" not in result1:
                assert target_course in result1, f"Result should mention course '{target_course}'"
            
            # Test with nonexistent course
            result2 = self.search_tool.execute("data", course_name="Nonexistent Course XYZ")
            print(f"\\n❌ Search in nonexistent course:")
            print(f"   Result: {result2}")
            
            assert "No course found matching" in result2, "Should indicate course not found"
...
class TestToolManagerWithRealData:
    ...
    def test_tool_execution_through_manager(self):
        ...
        result = self.tool_manager.execute_tool("search_course_content", query="retrieval")
    ...
    def test_sources_management(self):
        ...
        # Execute a search
        result = self.tool_manager.execute_tool("search_course_content", query="embedding")
